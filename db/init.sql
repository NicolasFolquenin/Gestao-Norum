-- ============================================================================
-- NORUM Engenharia â€” Schema PostgreSQL (produÃ§Ã£o)
-- Recomendado rodar em PostgreSQL gerenciado (Supabase, Neon ou AWS RDS)
-- com backups automÃ¡ticos e point-in-time recovery ativados.
-- ============================================================================

CREATE EXTENSION IF NOT EXISTS "pgcrypto"; -- gen_random_uuid()

-- ---------------------------------------------------------------------------
-- Tipos enumerados: garantem que valores invÃ¡lidos NUNCA entrem no banco.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Itens monitorados (extintores, caixas de gordura, caixas d'Ã¡guaâ€¦)
-- periodicidade_meses define de quanto em quanto tempo revisar.
-- validade Ã© calculada, mas armazenada para consulta rÃ¡pida e histÃ³rico.
-- ---------------------------------------------------------------------------
CREATE TABLE itens_monitorados (
    ...
    -- pertence a um condomÃ­nio OU Ã© avulso (com nome), nunca ambos vazios
...
-- ---------------------------------------------------------------------------
-- ServiÃ§os / agenda de manutenÃ§Ã£o + controle de nota fiscal e pagamento
-- Regras de consistÃªncia garantidas por CHECK (nÃ£o dÃ¡ para marcar "paga"
-- sem data de pagamento, nem "emitida" sem nÃºmero de nota).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Trigger: mantÃ©m atualizado_em sempre correto.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- View pronta: itens em alerta (vencidos ou vencendo em atÃ© 30 dias)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- View pronta: contas a receber (notas emitidas e nÃ£o pagas)
-- ---------------------------------------------------------------------------
